...
def get_env_stats_path(model_path: str) -> str:
    model_dir = os.path.dirname(model_path)
    model_base = os.path.splitext(os.path.basename(model_path))[0]
    candidates = [
        os.path.join(model_dir, f"{model_base}_env.pkl"),
        os.path.join(model_dir, 'quoridor_ppo_final_env.pkl'),
    ]
...
def run_bot(model_path: str, url: str, side: str, headless: bool):
    # 1. 創建環境並加載相關的統計數據
    base_env = QuoridorEnv()
    vec_env = DummyVecEnv([lambda: base_env])
    
    # 2. 載入環境統計數據 (提供更好的 AI 性能)
    stats_path = get_env_stats_path(model_path)
    if stats_path:
        vec_env = VecNormalize.load(stats_path, vec_env)
        vec_env.training = False       # 關閉訓練模式
        vec_env.norm_reward = False    # 關閉獎勵正規化
    ...
    with sync_playwright() as playwright:
        ...
        while step < 200:
            obs, mask, state = sync_env_from_page(page, base_env)
            last_state = state
            ...
            obs, mask, state = sync_env_from_page(page, base_env)
            win = base_env.board.check_win()
            print(f'執行後狀態：步數={step}, win={repr(win)}, valid_actions={sum(1 for v in mask if v)}')
            print(f'  board current={base_env.board.current_player.pos}, other={base_env.board.other_player.pos}')
